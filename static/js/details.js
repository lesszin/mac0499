...
function switchTab(tabName) {
    ...
    if (tabName === 'sheet') {
        ...
        document
            .getElementById('contentComparison')
            .style.display = 'block';

        document
            .getElementById('btnComparison')
...
window.initializeEvolution = initializeEvolution;
...
function createGroupCard(title, rows, emptyMessage = null) {
    ...
    if (rows.length === 0 && emptyMessage) {
        ...
        const hasSubgroups = rows.length > 0 && rows[0].subgroup;
        if (hasSubgroups) {
            rows
